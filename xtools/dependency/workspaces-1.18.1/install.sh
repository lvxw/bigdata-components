...
NETWORK_PLUGIN_BRANCH="release/1.4"
LOGGING_PLUGIN_BRANCH="release/1.0"
...
function set_listening_port(){
    ...
    then
        ...
        FILE=${KASM_INSTALL_BASE}/conf/app/agent/agent.app.config.yaml
        if [ -f "${FILE}" ]; then
            sed -i "s/public_port.*/public_port: ${DEFAULT_PROXY_LISTENING_PORT}/g" ${FILE}
        fi

        FILE=${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
        if [ -f "${FILE}" ]; then
...
function set_public_hostname() {
    sed -i "s/public_hostname.*/public_hostname: ${PUBLIC_HOSTNAME}/g" ${KASM_INSTALL_BASE}/conf/app/agent/agent.app.config.yaml
}
...
function set_database_hostname() {
    if [ "${DATABASE_HOSTNAME}" != "false" ] ;
        then
        sed -i "s/host: db/host: ${DATABASE_HOSTNAME}/g" ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
    fi
}

function set_database_port() {
    sed -i "s/port: 5432/port: ${DATABASE_PORT}/g" ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
}

function set_database_user() {
    ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '.database.username = "'${DATABASE_USER}'"' ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
    if [ "${ROLE}" == "all" ] || [ "${ROLE}" == "db" ];
    then
...
function set_database_name() {
    ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '.database.name = "'${DATABASE_NAME}'"' ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
    if [ "${ROLE}" == "all" ] || [ "${ROLE}" == "db" ];
    then
...
function set_database_password() {
    sed -i "s/ password:.*/ password: \"${DEFAULT_DATABASE_PASSWORD}\"/g" ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
...
function set_database_ssl() {
    if [ "${DATABASE_SSL}" == "false" ] ; then
        sed -i 's/ssl: true/ssl: false/' ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
...
function set_manager_hostname() {
    sed -i "s/hostnames: \['proxy.*/hostnames: \['${MANAGER_HOSTNAME}'\]/g" ${KASM_INSTALL_BASE}/conf/app/agent/agent.app.config.yaml
}

function set_manager_token() {
    sed -i "s/ token:.*/ token: \"${DEFAULT_MANAGER_TOKEN}\"/g" ${KASM_INSTALL_BASE}/conf/app/agent/agent.app.config.yaml
    FILE=${KASM_INSTALL_BASE}/conf/database/seed_data/default_properties.yaml
    if [ -f "${FILE}" ]; then
...
function configure_guac_component() {
    ...
    then
        ...
        then
            ...
            if [ -f "${FILE}" ] ;
            then
                sed -i "s/APIHOSTNAME/${API_SERVER_HOSTNAME}/g" "${FILE}"
                sed -i "s/ZONE/${SERVER_ZONE}/g" "${FILE}"
                sed -i "s/SERVER_ADDRESS/${PUBLIC_HOSTNAME}/g" "${FILE}"
                sed -i "s/SERVER_PORT/${DEFAULT_PROXY_LISTENING_PORT}/g" "${FILE}"
                sed -i "s/REGISTRATION_TOKEN/${REGISTRATION_TOKEN}/g" "${FILE}"
                echo "Successfully updated kasmguac.app.config.yaml"
            else
    ...
    then
        sed -i "s/APIHOSTNAME/proxy/g" ${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
        sed -i "s/SERVER_ADDRESS/proxy/g" ${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
        sed -i "s/SERVER_PORT/${DEFAULT_PROXY_LISTENING_PORT}/g" ${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
        sed -i "s/ZONE/${SERVER_ZONE}/g" ${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
        sed -i "s/REGISTRATION_TOKEN/${REGISTRATION_TOKEN}/g" ${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
    fi
}
...
function configure_rdp_gateway_component() {
    ...
    then
        if [ "${API_SERVER_HOSTNAME}" != "false" ] && [ "${PUBLIC_HOSTNAME}" != "false" ] && [ "${REGISTRATION_TOKEN}" != "false" ] ;
        then
            FILE=${KASM_INSTALL_BASE}/conf/app/rdp_gateway/passthrough.app.config.yaml
            if [ -f "${FILE}" ] ;
            then
                sed -i "s/00000000-0000-0000-0000-000000000000/${DEFAULT_RDP_GATEWAY_CONNECTION_PROXY_ID}/g" "${FILE}"
                sed -i "s/APIHOSTNAME/${API_SERVER_HOSTNAME}/g" "${FILE}"
                sed -i "s/ZONE/${SERVER_ZONE}/g" "${FILE}"
                sed -i "s/SERVER_ADDRESS/${PUBLIC_HOSTNAME}/g" "${FILE}"
                sed -i "s/SERVER_PORT/${DEFAULT_PROXY_LISTENING_PORT}/g" "${FILE}"
            ...
            FILE=${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdp_https_gateway.app.config.yaml
            if [ -f "${FILE}" ] ;
            then
                sed -i "s/00000000-0000-0000-0000-000000000000/${DEFAULT_RDP_HTTPS_GATEWAY_CONNECTION_PROXY_ID}/g" "${FILE}"
                sed -i "s/APIHOSTNAME/${API_SERVER_HOSTNAME}/g" "${FILE}"
                sed -i "s/ZONE/${SERVER_ZONE}/g" "${FILE}"
                sed -i "s/SERVER_ADDRESS/${PUBLIC_HOSTNAME}/g" "${FILE}"
                sed -i "s/SERVER_PORT/${DEFAULT_PROXY_LISTENING_PORT}/g" "${FILE}"
    ...
    then
        sed -i "s/00000000-0000-0000-0000-000000000000/${DEFAULT_RDP_GATEWAY_CONNECTION_PROXY_ID}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_gateway/passthrough.app.config.yaml
        sed -i "s/APIHOSTNAME/proxy/g" ${KASM_INSTALL_BASE}/conf/app/rdp_gateway/passthrough.app.config.yaml
        sed -i "s/SERVER_ADDRESS/proxy/g" ${KASM_INSTALL_BASE}/conf/app/rdp_gateway/passthrough.app.config.yaml
        sed -i "s/SERVER_PORT/${DEFAULT_PROXY_LISTENING_PORT}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_gateway/passthrough.app.config.yaml
        sed -i "s/ZONE/${SERVER_ZONE}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_gateway/passthrough.app.config.yaml
        sed -i "s/REGISTRATION_TOKEN/${REGISTRATION_TOKEN}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_gateway/passthrough.app.config.yaml
        echo "Successfully updated passthrough.app.config.yaml"
        sed -i "s/00000000-0000-0000-0000-000000000000/${DEFAULT_RDP_HTTPS_GATEWAY_CONNECTION_PROXY_ID}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdp_https_gateway.app.config.yaml
        sed -i "s/APIHOSTNAME/proxy/g" ${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdp_https_gateway.app.config.yaml
        sed -i "s/SERVER_ADDRESS/proxy/g" ${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdp_https_gateway.app.config.yaml
        sed -i "s/SERVER_PORT/${DEFAULT_PROXY_LISTENING_PORT}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdp_https_gateway.app.config.yaml
        sed -i "s/ZONE/${SERVER_ZONE}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdp_https_gateway.app.config.yaml
        sed -i "s/REGISTRATION_TOKEN/${REGISTRATION_TOKEN}/g" ${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdp_https_gateway.app.config.yaml
        echo "Successfully updated rdp_https_gateway.app.config.yaml"
    fi
}
...
function set_guac_cluster_size() {
    ...
    sed -i "s/CLUSTER_SIZE/${GUAC_CLUSTER_SIZE}/g" ${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
...
function set_agent_server_id() {
    ...
    sed -i "s/server_id.*/server_id: ${SERVER_ID}/g" ${KASM_INSTALL_BASE}/conf/app/agent/agent.app.config.yaml
...
function set_server_zone() {
    ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.server.zone_name) = "'${SERVER_ZONE}'"' ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
...
function set_api_server_id() {
    ...
    sed -i "s/server_id.*/server_id: ${API_SERVER_ID}/g" ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
}

function set_manager_id() {
    MANAGER_ID=$(cat /proc/sys/kernel/random/uuid)
    sed -i "s/manager_id.*/manager_id: ${MANAGER_ID}/g" ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
}

function set_share_id() {
    SHARE_ID=$(cat /proc/sys/kernel/random/uuid)
    sed -i "s/share_id.*/share_id: ${SHARE_ID}/g" ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
}
...
function set_api_hostname() {
    ...
    sed -i "s/server_hostname.*/server_hostname: ${API_SERVER_HOSTNAME}/g" ${KASM_INSTALL_BASE}/conf/app/api/api.app.config.yaml
}

function set_provider() {
    if [ "${PROVIDER}" != "false" ] ;
    then
        sed -i "s/provider.*/provider: ${PROVIDER}/g" ${KASM_INSTALL_BASE}/conf/app/agent/agent.app.config.yaml
    fi
}
...
function base_install() {
    ...
    # This directory may be used on non-db hosts when running db utility scripts remotely
    chown 70:70 ${KASM_INSTALL_BASE}/tmp/kasm_db
...
function copy_manager_configs() {
    ...
    cp -r ${KASM_RELEASE}/conf/nginx/services.d/closest.conf ${KASM_INSTALL_BASE}/conf/nginx/services.d/closest.conf
}
...
function copy_guac_configs() {
    ...
    cp -r ${KASM_RELEASE}/conf/app/guac/kasmguac.app.config.yaml ${KASM_INSTALL_BASE}/conf/app/guac/kasmguac.app.config.yaml
}
...
function copy_rdpproxy_configs() {
  ...
  then
    ...
    then
        ...
        cp -r ${KASM_RELEASE}/conf/app/rdp_gateway/rdpproxy.ini ${KASM_INSTALL_BASE}/conf/app/rdp_gateway/rdpproxy.ini
    fi
    cp -r ${KASM_RELEASE}/conf/app/rdp_https_gateway/rdpgw.yaml ${KASM_INSTALL_BASE}/conf/app/rdp_https_gateway/rdpgw.yaml
  elif [ "${ROLE}" == "app" ] ;
  then
...
function restart_docker() {
    if [ ! -f "$SKIP_DOCKER_RESTART_FLAG_FILE" ]; then
        ...
        sleep 10s && sudo service docker restart
    else
        echo "Skipping docker engine restart"
    fi
}

function install_logger_plugin() {
    echo "Installing docker logging plugin"
    local logging_plugin_alias="kasmweb/logger"

    if [ ! -z "$LOGGER_PLUGIN_IMAGE_TARFILE" ]; then
        echo "Loading logger plugin from offline tarball: $LOGGER_PLUGIN_IMAGE_TARFILE"
        tar -zxf "$LOGGER_PLUGIN_IMAGE_TARFILE" -C /var/lib/docker/plugins
        echo "Restarting docker engine"
        sleep 10s && sudo service docker restart

        OFFLINE_LOGGER_PLUGIN_NAME=$(sudo docker plugin ls --format '{{.Name}}' | grep -E "${logging_plugin_alias}" | head -n1)
        if [ -z "$OFFLINE_LOGGER_PLUGIN_NAME" ]; then
            echo "ERROR: Could not find a kasm logger plugin to enable after offline extraction."
            cleanup_log
            exit 1
        fi

        sudo docker plugin enable "$OFFLINE_LOGGER_PLUGIN_NAME" &> /dev/null || true
    else
      local logging_plugin_tag
      local logging_plugin_image="kasmweb/kasm-logging-plugin"
      if [[ "${LOGGING_PLUGIN_BRANCH}" != release/* ]] && [[ "${LOGGING_PLUGIN_BRANCH}" != develop ]]; then
        logging_plugin_image="${logging_plugin_image}-private"
      fi

      logging_plugin_tag="${ARCH}-$(echo $LOGGING_PLUGIN_BRANCH | sed -r 's#^release/##'  | sed 's/\//_/g')"
      if [[ "${USE_ROLLING}" == "true" ]]; then
          logging_plugin_tag="${logging_plugin_tag}-rolling-daily"
      fi

      echo "Installing docker logging plugin: ${logging_plugin_image}"

      local output
      output=$(sudo docker plugin install --alias "${logging_plugin_alias}:${logging_plugin_tag}" "$logging_plugin_image:${logging_plugin_tag}" --grant-all-permissions)
      if [[ $? -ne 0 ]]; then
        echo "${output}"
        echo "Failed to install Kasm Logging Plugin"
        exit 1
      fi
      echo "${output}"

      sudo docker plugin enable "$logging_plugin_alias" &> /dev/null || true
    fi

    echo "Logger plugin installation complete."
}

function remove_logger_plugin() {
    # remove the plugin
    CURRENT_LOGGER_PLUGIN_NAME=$(sudo docker plugin ls | awk '/kasmweb\/.*logger/{print $2}')
    if [ ! -z "${CURRENT_LOGGER_PLUGIN_NAME}" ]; then
        sudo docker plugin disable ${CURRENT_LOGGER_PLUGIN_NAME} &> /dev/null || true
        sudo docker plugin rm ${CURRENT_LOGGER_PLUGIN_NAME} &> /dev/null || true
    fi

    # remove temporary files
    rm -rf /var/run/kasm-logger
    rm -rf /var/log/kasm-logger
}
...
function remove_network_sidecar_plugin() {
    ...
    # remove the plugin
    CURRENT_NETWORK_PLUGIN_NAME=$(sudo docker plugin ls | awk '/kasmweb\/.*sidecar/{print $2}')
...
function install_network_sidecar_plugin() {
    local network_plugin_alias="kasmweb/sidecar"
    ...
    if [ -n "${NETWORK_PLUGIN_IMAGE_TARFILE}" ]; then
        echo "Loading sidecar plugin from offline tarball: $NETWORK_PLUGIN_IMAGE_TARFILE"
        tar -zxf "$NETWORK_PLUGIN_IMAGE_TARFILE" -C /var/lib/docker/plugins
        echo "Restarting docker engine"
        sleep 10s && sudo service docker restart

        OFFLINE_NETWORK_PLUGIN_NAME=$(sudo docker plugin ls --format '{{.Name}}' | grep -E "${network_plugin_alias}" | head -n1)
        if [ -z "$OFFLINE_NETWORK_PLUGIN_NAME" ]; then
            echo "ERROR: Could not find a kasm logger plugin to enable after offline extraction."
            cleanup_log
            exit 1
        fi
        ...
        local network_plugin_tag
        local network_plugin_image="kasmweb/kasm-network-plugin"
        if [[ "${NETWORK_PLUGIN_BRANCH}" != release/* ]] && [[ "${NETWORK_PLUGIN_BRANCH}" != develop ]]; then
          network_plugin_image="${network_plugin_image}-private"
        fi

        network_plugin_tag="${ARCH}-$(echo $NETWORK_PLUGIN_BRANCH | sed -r 's#^release/##'  | sed 's/\//_/g')"
        if [[ "${USE_ROLLING}" == "true" ]]; then
            network_plugin_tag="${network_plugin_tag}-rolling-daily"
        fi

        local timestamp=$(TZ=UTC date -Iseconds | awk -F "+" '{print $1}')Z
        local output
        output=$(sudo docker plugin install --alias "${network_plugin_alias}:${network_plugin_tag}" "${network_plugin_image}:${network_plugin_tag}" --grant-all-permissions)
        if [[ $? -ne 0 ]]; then
          echo "${output}"
          echo "Failed to install Kasm Network Plugin"
          exit 1
        fi
        echo "${output}"
        local digest
        ...
        local stable_version=$(bash ${SCRIPT_PATH}/bin/utils/plugin_helper "${network_plugin_image}:${network_plugin_tag}" --latest)
        echo "${timestamp},${stable_version:-$network_plugin_tag},${digest}" > "${KASM_INSTALL_BASE}/plugins/$(echo "${network_plugin_alias}" | sed -r 's#[./:-]#_#g').csv"
        sudo docker plugin enable ${network_plugin_alias} &> /dev/null || true
...
function create_sidecar_network() {
    ...
    if [ $ret -ne 0 ]; then
        local network_plugin_name=$(sudo docker plugin ls | awk '/kasmweb\/sidecar/{print $2}')
...
function connectivity_test() {
	...
	if [ "${POSTGRES_CONNECTED}" == "true" ];
        then
            echo "Database connectivity passed ${DATABASE_HOSTNAME} on ${DATABASE_PORT}"
        else
...
function display_help() {
    ...
    echo -e "| ${CMD}-T|--db-port${NC}                | Database port for external DB (default 5432)                    |"
    ...
    echo -e "| ${CMD}-x|--offline-network-plugin${NC} | Path to the tar.gz network plugin images offline installer      |"
    echo -e "| ${CMD}--offline-logger-plugin${NC}     | Path to the tar.gz logger plugin images offline installer       |"
...
function load_workspace_images () {
    ...
    while IFS="" read -r image || [ -n "$image" ]; do
        ...
        podman push $image  docker-daemon:$image
...
function configure_systemd_services () {
    if ! can_use_systemd; then
        echo "systemd not available, skipping systemd service configuration"
        return
    fi
    ...
    if { [ "${ROLE}" == "all" ] || [ "${ROLE}" == "agent" ]; } && [ "${SKIP_EGRESS}" != "true" ]; then
...
for index in "${!ARGSAPPEND[@]}"; do
  case ${ARGSAPPEND[index]} in
    ...
    --offline-logger-plugin)
        LOGGER_PLUGIN_IMAGE_TARFILE="${ARGSAPPEND[index+1]}"
...
mkdir -p ${KASM_INSTALL_BASE}/plugins
mkdir -p ${KASM_INSTALL_BASE}/tmp/api
mkdir -p ${KASM_INSTALL_BASE}/tmp/kasm_db
...
mkdir -p ${KASM_INSTALL_BASE}/healthcheck/kasm_api
mkdir -p ${KASM_INSTALL_BASE}/healthcheck/kasm_agent
mkdir -p ${KASM_INSTALL_BASE}/healthcheck/kasm_manager
mkdir -p ${KASM_INSTALL_BASE}/healthcheck/kasm_guac
mkdir -p ${KASM_INSTALL_BASE}/healthcheck/rdp_gateway
mkdir -p ${KASM_INSTALL_BASE}/healthcheck/rdp_https_gateway
...
then
    ...
    #Only develop for db image
    sed -i 's/postgres:develop-rolling/postgres:develop/g' ${KASM_INSTALL_BASE}/docker/.conf/*.yaml
fi
...
then
    sed -i '/ name:/ s/$/-rolling-daily/' ${KASM_INSTALL_BASE}/conf/database/seed_data/default_images_amd64.yaml
    sed -i '/ name:/ s/$/-rolling-daily/' ${KASM_INSTALL_BASE}/conf/database/seed_data/default_images_arm64.yaml
    sed -i '/ image:/ s/"$/-staging"/' ${KASM_INSTALL_BASE}/docker/.conf/*.yaml

    # accounting for hardcoded version and using the rolling image since these projects are not maintained in the backend repo
    sed -i 's|\(kasmweb/rdp-gateway:.*\)-.*|\1-rolling"|g' ${KASM_INSTALL_BASE}/docker/.conf/*.yaml
    sed -i 's|\(kasmweb/rdp-https-gateway:.*\)-.*|\1-rolling"|g' ${KASM_INSTALL_BASE}/docker/.conf/*.yaml
    sed -i 's|\(kasmweb/postgres:.*\)-.*|\1-rolling"|g' ${KASM_INSTALL_BASE}/docker/.conf/*.yaml
...
then
    ...
    remove_logger_plugin
    install_logger_plugin
    restart_docker
...
then
    ...
    if can_use_systemd; then
        ...
        if [ "${SKIP_EGRESS}" != "true" ] ; then
            ${KASM_INSTALL_BASE}/bin/start_network_plugin
        fi
