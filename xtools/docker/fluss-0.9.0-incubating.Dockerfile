...
RUN sed -i 's|"$FLUSS_BIN_DIR"/fluss-daemon.sh start zookeeper|#"$FLUSS_BIN_DIR"/fluss-daemon.sh start zookeeper|' ${FLUSS_HOME}/bin/local-cluster.sh && \
    cp -r ${FLUSS_HOME}/plugins/hdfs/fluss-fs-hdfs-0.9.0-incubating.jar ${FLUSS_HOME}/lib/ && \
...
RUN echo '#!/bin/bash' > /usr/local/bin/enterpoint.sh && \
    ...
    echo 'application_id=`yarn application -list  |  grep fluss-lake-tiering-service  |  awk '"'"'{print $1}'"'"'`'  >> /usr/local/bin/enterpoint.sh && \
